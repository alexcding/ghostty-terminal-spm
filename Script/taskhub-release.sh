#!/usr/bin/env bash
# Builds TaskHub's release artifacts from the pinned Ghostty revision:
#   build/release/GhosttyKit.xcframework.zip   the binary target of this package
#   build/release/ghostty-vt-runtime.zip       the headless VT runtime taskhub-ptyd links
...
cd "$(dirname "$0")/.."
ROOT="$PWD"
LOCK="$ROOT/TaskHub/ghostty.lock.json"
BUILD="$ROOT/build"
...
for patch in "$ROOT"/TaskHub/patches/native/*.patch; do git -C "$SRC" apply "$patch"; done
...
for patch in "$ROOT"/TaskHub/patches/vt/*.patch; do git -C "$VT" apply "$patch"; done
...
echo "$REVISION" > "$RUNTIME/taskhub-ghostty-revision"
cp "$ROOT/TaskHub/patches/vt/0003-terminal-query-validation.patch" "$RUNTIME/taskhub-ghostty-query-patch"
cp "$ROOT/TaskHub/patches/vt/0007-glyph-snapshot.patch" "$RUNTIME/taskhub-ghostty-glyph-patch"
cp "$ROOT/TaskHub/patches/vt/0008-graphics-snapshot.patch" "$RUNTIME/taskhub-ghostty-graphics-patch"
